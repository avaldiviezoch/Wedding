...
function runtime() {
  ...
  const context = {
    ...
    document: {
      ...
      getElementById(id) { return id === 'canvasWrap' ? canvasWrap : null; }
    },
    window: {},
    requestAnimationFrame(fn) { fn(); },
    currentScale() { return context.scale; },
    scale: 32,
    planner,
    drawLayer,
    guideLayer,
    ...
    rectPolygon(item) {
      ...
      const hw = item.widthM * s / 2, hh = item.heightM * s / 2;
    ...
    polygonIntersectsPolygon() { return false; },
    intersects() { return false; }
  };
  vm.createContext(context);
  vm.runInContext(source, context, { filename: 'phase2-sanitize.js' });
  return { context, api: context.window.MiGranDiaDistributionSanitization, planner, canvasWrap };
}

function squareMeters(centerXM, centerYM, sizeM, scale) {
  const h = sizeM * scale / 2;
  const x = centerXM * scale, y = centerYM * scale;
...
test('tolerancias son físicas y conservan el mismo umbral en escalas 18, 32 y 50 px/m', () => {
  const { api } = runtime();
  ...
  for (const scale of [18, 32, 50]) {
    ...
    const a = squareMeters(2, 2, 1, scale);
    const bOverlap = squareMeters(2.8, 2, 1, scale);
    const bGap = squareMeters(3.2, 2, 1, scale);
    assert.equal(api.satIntersects(a, bOverlap, scale), true);
    assert.equal(api.satIntersects(a, bGap, scale), false);
  }
});

test('SAT mantiene resultados coherentes con rectángulos rotados', () => {
  const { api } = runtime();
  for (const scale of [18, 32, 50]) {
...
test('capas ocultas son visuales: conservan capacidad/asignaciones y salen de conflicto/proximidad', () => {
  const { api } = runtime();
...
test('límites usan la geometría completa y no solo el centro', () => {
  const { api } = runtime();
...
test('restore/undo limpia selección inválida y estados transitorios', () => {
  const { context } = runtime();
  context.measureDraft = { start:{x:1,y:1}, end:{x:2,y:2} };
  context.drawingTent = true;
  context.tentDraft = [{x:1,y:1}];
  context.tentHoverPoint = {x:2,y:2};
  context.guideLines = { vertical: 100, horizontal: 200 };
  context.restoreState({
    elements: [{ id:'visible', type:'bar', shape:'rect', x:200, y:200, widthM:1, heightM:1 }],
    selectedIds: ['missing','visible'],
    selectedId: 'missing',
    hiddenLayers: {}
  });
...
test('touch mantiene un ancla de viewport durante pinch', () => {
