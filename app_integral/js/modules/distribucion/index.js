...
const tableSig=r=>JSON.stringify(lt(r?.data||{}).map(e=>({id:e.id,label:e.label,capacity:e.capacity})));
const linkSig=r=>JSON.stringify({
 guests:(r?.data?.guests||[]).map(g=>({id:g.id,name:g.name,sourceGuestId:g.sourceGuestId,status:g.status,rsvpStatus:g.rsvpStatus})),
 tables:lt(r?.data||{}).map(e=>({
  id:e.id,
  label:e.label,
  capacity:e.capacity,
  seats:Array.isArray(e.seats)?e.seats:[],
  sharedTableId:e.sharedTableId,
  sharedTableType:e.sharedTableType
 }))
});
...
function patchLegacyTableDom(c,d,l,p){
 try{
  const doc=c.frame.contentDocument;
  const layer=doc?.getElementById('itemsLayer');
  if(!layer||!p?.tables)return;
  const tableMap=new Map((d.tables||[]).map(t=>[String(t.id),t]));
  const groups=[...layer.querySelectorAll('.draggable[data-id]')];
  Object.entries(p.tables).forEach(([cid,eid])=>{
   const table=tableMap.get(String(cid)); if(!table)return;
   const n=cap(table.capacity||table.seats?.length||10);
   const group=groups.find(g=>String(g.getAttribute('data-id'))===String(eid)); if(!group)return;

   [...group.children].filter(node=>String(node.tagName).toLowerCase()==='text').forEach(node=>{
    if(/^\d+\s+personas$/i.test(String(node.textContent||'').trim())) node.textContent=`${n} personas`;
   });

   const chairs=[...group.children].filter(node=>{
    if(String(node.tagName).toLowerCase()!=='g')return false;
    const circle=[...node.children].find(x=>String(x.tagName).toLowerCase()==='circle');
    const text=[...node.children].find(x=>String(x.tagName).toLowerCase()==='text'&&/^\d+$/.test(String(x.textContent||'').trim()));
    return Boolean(circle&&text);
   }).sort((a,b)=>Number(a.querySelector('text')?.textContent||0)-Number(b.querySelector('text')?.textContent||0));

   let chairOrbit=0;
   if(chairs[0]){
    const circle=chairs[0].querySelector('circle');
    chairOrbit=Math.hypot(Number(circle?.getAttribute('cx')||0),Number(circle?.getAttribute('cy')||0));
   }
   chairs.forEach((chair,index)=>{
    if(index>=n){chair.style.display='none';return}
    chair.style.display='';
    if(!chairOrbit)return;
    const angle=(Math.PI*2*index/n)-Math.PI/2;
    const x=Math.cos(angle)*chairOrbit,y=Math.sin(angle)*chairOrbit;
    const circle=chair.querySelector('circle'), text=chair.querySelector('text');
...
function deferPush(c){clearTimeout(c.pushTimer);c.pushTimer=setTimeout(()=>push(c),220)}
function refreshLiveFrame(c){
 clearTimeout(c.reloadTimer);
 c.reloadTimer=setTimeout(()=>{
  if(!c.frame.isConnected)return;
  const state=plannerSaveState(c);
  if(state==='saving'){refreshLiveFrame(c);return}
  if(state==='error'){console.warn('Distribución: se evitó refrescar porque hay cambios sin guardar.');return}
  try{c.frame.contentWindow.location.reload()}catch(_){}
 },120);
}
async function push(c){if(c.busy||!c.frame.isConnected)return;const state=plannerSaveState(c);if(state==='saving'){deferPush(c);return}if(state==='error'){console.warn('Distribución: sincronización pospuesta por un error de guardado.');return}c.busy=true;try{const p=await planner(c.frame.contentWindow);if(!p.r?.data||!p.aid)return;const d=read(),l=lread(),before=proj(p.r),beforeTables=tableSig(p.r),nr=cp(p.r);nr.data=cp(nr.data);pushSnap(nr.data,d,l,p.aid);lsave(l);c.aid=p.aid;if(before!==proj(nr)){const tablesChanged=beforeTables!==tableSig(nr),wr=await writePlan(c.frame.contentWindow,p,nr);c.last=wr?.updatedAt||'';c.link=linkSig(nr);if(wr&&tablesChanged)refreshLiveFrame(c)}else{c.last=p.r.updatedAt||'';c.link=linkSig(p.r)}setTimeout(()=>patchLegacyTableDom(c,d,l,pe(l,p.aid)),40)}finally{c.busy=false}}
async function pull(c,initial=false){if(c.busy||!c.frame.isConnected)return; c.busy=true;try{const p=await planner(c.frame.contentWindow);if(!p.r?.data||!p.aid)return;const old=read(),l=lread(),d=pullSnap(p.r.data,old,l,p.aid,initial);pe(l,p.aid).initialized=true;lsave(l);if(sig(old)!==sig(d))save(d,initial?'distribucion-migration':'distribucion-seating');c.aid=p.aid;c.last=p.r.updatedAt||'';c.link=linkSig(p.r)}finally{c.busy=false}}
...
function bind(f){if(!(f instanceof HTMLIFrameElement))return;let d;try{d=f.contentDocument}catch(_){return}if(!isDist(d))return;let c=ctl.get(f);if(!c){c={frame:f,aid:'',last:'',link:'',busy:false,init:false,timer:0,reloadTimer:0,pushTimer:0,doc:null};ctl.set(f,c)}if(c.doc===d)return;c.doc=d;clearInterval(c.timer);if(d.documentElement.dataset.mgdDistGuestLink!==V){d.documentElement.dataset.mgdDistGuestLink=V;d.addEventListener('change',e=>{const s=e.target?.closest?.('#seatEditor select[data-seat-index]');if(s){seatChange(c,s);setTimeout(()=>poll(c),420)}},true)}c.timer=setInterval(()=>poll(c).catch(console.warn),800);init(c).catch(e=>console.warn('Distribución: vínculo con Invitados',e))}
...
document.addEventListener('visibilitychange',()=>{if(document.hidden||!String(location.hash||'').toLowerCase().includes('distribucion'))return;clearTimeout(resumeTimer);resumeTimer=setTimeout(()=>document.getElementById('unifiedWorkspace')?.querySelectorAll('iframe').forEach(f=>{const c=ctl.get(f);if(c&&!c.busy&&!c.init)poll(c).catch(console.warn)}),500)});
