...
const NATIVE_WIDGET_URL = 'https://avaldiviezoch.github.io/Wedding/app_integral/js/modules/invitados/rsvp-native-widget.js?v=20260819-1900-firestore-music1';
...
function renderPreview(doc, config = readBuilder(doc)) {
  ...
  const c = normalizeMusicConfig(config);
  host.innerHTML = `<small>Vista previa</small><strong>${escapeHtml(c.title)}</strong><p>${escapeHtml(c.intro)}</p><div class="mgd-music-preview-fields"><div class="mgd-music-preview-field">Canción</div>${c.askArtist ? '<div class="mgd-music-preview-field">Artista</div>' : ''}${c.askMessage ? `<div class="mgd-music-preview-field wide">${escapeHtml(c.messageLabel)}</div>` : ''}</div>`;
}
...
function annotateResponseCards(doc, items) {
  ...
  doc.querySelectorAll('[data-rsvp-response-id]').forEach((card) => {
    ...
    if (main) { const box=doc.createElement('div'); box.className='rsvp-music-response-box'; box.dataset.mgdMusicBox='true'; const lines=[]; if(songs.length) lines.push(`<strong>Música:</strong> ${songs.map((song)=>escapeHtml([song.title,song.artist].filter(Boolean).join(' — '))).join(' · ')}`); if(music.message) lines.push(`<strong>Mensaje:</strong> ${escapeHtml(music.message)}`); box.innerHTML=lines.join('<br>'); main.appendChild(box); }
  });
}
