...
(() => {
  ...
  function applyMobileZoom(){
    svg.style.width=Math.round(mobileBaseWidth*zoom)+'px';
    svg.style.height='auto';
    document.getElementById('zoomReset').textContent=Math.round(zoom*100)+'%';
  }

  /*
    Un dedo sobre el fondo desplaza el plano.
    Un dedo sobre un objeto conserva la edición del objeto.
    Dos dedos acercan o alejan manteniendo el punto central.
  */
  let mobilePanGesture=null;
  ...
  function startMobilePinch(event){
    if(event.touches.length<2 || !canvasWrap) return;
    ...
    drag=null;
    guideLines={vertical:null,horizontal:null};
    mobilePanGesture=null;

    mobilePinchGesture={
      startDistance:Math.max(1,touchDistance(first,second)),
      startZoom:zoom,
      contentX:(canvasWrap.scrollLeft+midpoint.x-rect.left)/zoom,
      contentY:(canvasWrap.scrollTop+midpoint.y-rect.top)/zoom
    };
  }
  ...
  canvasWrap.addEventListener('touchstart',event=>{
    ...
    if(
      ...
      event.target.closest(
        '.draggable,.rotate-handle,.vertex-handle,.canvas-toolbar,.floating-legend'
      )
    ){
      mobilePanGesture=null;
      return;
  ...
  canvasWrap.addEventListener('touchmove',event=>{
    if(event.touches.length>=2){
      ...
      const midpoint=touchMidpoint(first,second);
      const rect=canvasWrap.getBoundingClientRect();
      const currentDistance=Math.max(1,touchDistance(first,second));

      zoom=Math.max(
        .55,
        Math.min(
          1.8,
          mobilePinchGesture.startZoom*
          (currentDistance/mobilePinchGesture.startDistance)
        )
      );
      ...
      canvasWrap.scrollLeft=
        mobilePinchGesture.contentX*zoom-
        (midpoint.x-rect.left);

      canvasWrap.scrollTop=
        mobilePinchGesture.contentY*zoom-
        (midpoint.y-rect.top);

      return;
    }

    if(event.touches.length===1 && mobilePanGesture){
      event.preventDefault();

      const touch=event.touches[0];
      canvasWrap.scrollLeft=
        mobilePanGesture.startScrollLeft-
        (touch.clientX-mobilePanGesture.startX);

      canvasWrap.scrollTop=
        mobilePanGesture.startScrollTop-
        (touch.clientY-mobilePanGesture.startY);
    }
  },{passive:false});

  function finishMobileTouch(event){
    if(event.touches.length<2) mobilePinchGesture=null;
    if(event.touches.length===0) mobilePanGesture=null;
  }

  canvasWrap.addEventListener('touchend',finishMobileTouch,{passive:true});
  canvasWrap.addEventListener('touchcancel',finishMobileTouch,{passive:true});
  ...
  mobileUndo.addEventListener('click',()=>btnUndo.click());
  mobileRedo.addEventListener('click',()=>btnRedo.click());

  const historyButtonObserver=new MutationObserver(()=>{
    mobileUndo.disabled=btnUndo.disabled;
    mobileRedo.disabled=btnRedo.disabled;
  });

  historyButtonObserver.observe(btnUndo,{attributes:true,attributeFilter:['disabled']});
  historyButtonObserver.observe(btnRedo,{attributes:true,attributeFilter:['disabled']});
    ...
    requestAnimationFrame(()=>{
      if(canvasWrap){
        canvasWrap.scrollLeft=Math.max(
          0,
          (svg.scrollWidth-canvasWrap.clientWidth)*.42
        );
        canvasWrap.scrollTop=90;
      }
    });
  });
})();
